...
的处理结果发送如下成功或失败信息*/
static const char *szret[] = {"I get a correct resoult\n", "something wrong\n"};
/*从状态机，用于解析出一行内容*/
...
/*分析头部字段*/
HTTP_CODE parese_headers(char *temp) {
...
HTTP_CODE parse_content(char *buffer, int &checked_index,
                        CHECK_STATE &checkstate, int &read_inex,
                        int &start_line) {
  LINE_STATUS linestatus = LINE_OK; /*记录当前行的读取状态*/
  HTTP_CODE retcode = NO_REQUEST;   /*记录HTTP请求的处理结果*/
  /*主状态机，用于从buffer中取出所有完整的行*/
  while ((linestatus = parse_line(buffer, checked_index, read_inex)) ==
         LINE_OK) {
